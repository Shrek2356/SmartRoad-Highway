...
| 使用方式 | 基础环境 | 是否需要模型 |
|----------|----------|--------------|
| v1.4.4 完整桌面包，仅演示 | 随包 Python、业务依赖与前端构建；无需系统 Node | 不需要 |
| GitHub 源码开发 | 自备 Python／Node，安装开发与业务依赖 | 只做 Demo 时不需要 |
| 本地离线／云端视觉加本地定位 | 完整检测环境，按显卡配置 CUDA PyTorch 与其他依赖 | 需要对应外部模型；云端还需 API 配置 |

完整包的 Windows 独立窗口需要 WebView2 Runtime。本源码仓库不包含 Python/Node 运行环境、EXE、前端构建产物或模型；请按源码开发方式准备环境。
...
1. 完整解压到较短的目录，关闭旧版，再打开根目录 `SmartRoad-Inspection.exe`。
2. 登录后提交道路样例并核对任务结果；Demo 不需要重新安装基础 Python 或 Node。
...
权重不包含在代码／演示包里；按部署指南取得匹配的 Qwen GGUF 与 mmproj、SAM3，以及可选 CLIP。云端视觉服务不替代所有本地定位依赖。
